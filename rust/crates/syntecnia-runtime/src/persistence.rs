...
//! `syntecnia/runtime/persistence.py`.
//!
...
//! Ruta: `~/.syntecnia/state/<program_name>.db` (idéntico al oráculo; mismo esquema,
...
use syntecnia_agents::memory::{AgentMemory, MemoryCategory, MemoryEntry, OwnerRule, RuleLevel};
use syntecnia_agents::progress::{ProgressManager, StepStatus, TaskProgress, TaskStep};

fn default_state_path(program_name: &str) -> PathBuf {
    let home = std::env::var("USERPROFILE")
        .or_else(|_| std::env::var("HOME"))
        .unwrap_or_default();
    let dir = PathBuf::from(home).join(".syntecnia").join("state");
...
impl StatePersistence {
    /// Abre (o crea) la base para `program_name` bajo `~/.syntecnia/state/`.
