//! Motor de ejecución. Análogo de `SyntecniaEngine.run_source`.
//!
//! Dos caminos:
...
use syntecnia_llm::human::{AutoHandler, InteractionManager};
use syntecnia_llm::provider::{LLMProvider, LLMRequest, MockProvider};

use syntecnia_agents::builtins::register_agent_builtins;
use syntecnia_agents::memory::AgentMemory;
use syntecnia_agents::progress::ProgressManager;
use syntecnia_agents::swarm::{AgentState, Swarm};
use syntecnia_capabilities::model::{
    capability_type_from_name, Capability, CapabilitySet, CapabilityType,
};
use syntecnia_capabilities::secure::register_secure_builtins;
use syntecnia_core::ast::Node;
use syntecnia_core::interpreter::{Control, Interpreter, RunResult, SwarmHooks};
use syntecnia_core::parser::{parse_source, CompileError};
use syntecnia_core::types::{from_send, to_send, SendValue, SynValue};
use syntecnia_stdlib::cron::{register_cron_builtins, CronScheduler};
use syntecnia_stdlib::database::{register_database_builtins, DatabaseManager};
use syntecnia_stdlib::http::register_http_builtins;
...
pub(crate) fn wire_common_with_state(
    ...
    syntecnia_stdlib::server::register_serve_builtins(interp);
    ...
    interp.register_builtin(
        ...
        Rc::new(|i, args, _loc| {
            ...
            let html = syntecnia_core::templates::render_template(i, &path, args.get(1))?;
            Ok(syntecnia_core::templates::make_raw(html, "text/html; charset=utf-8", 200))
        }),
    );
    let c = caps.clone();
...
pub fn repl() {
    ...
    println!("Syntecnia REPL — escribí sentencias; Ctrl+Z (Windows) / Ctrl+D para salir.");
    let stdin = io::stdin();
