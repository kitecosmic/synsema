//! Syntecnia CLI. Espeja `syntecnia/cli.py` y `__main__.py` (capa 11, en progreso).
//!
//! Subcomando de conformidad (gate de paridad contra el oráculo Python):
//!
//!     syntecnia-cli conform <archivo.syn>
//!
...
use syntecnia_runtime::daemon;
use syntecnia_runtime::engine::{repl, run_source, run_swarm_dump};
use syntecnia_runtime::serve::run_serve_program;

const USAGE: &str = "uso: syntecnia-cli <conform [--swarm] [--flat] | serve [--secure] | run | check | tokens | ast | repl | daemon | version> <archivo.syn>";
...
fn main() -> ExitCode {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("version") | Some("--version") | Some("-V") => {
            println!("Syntecnia v{}", env!("CARGO_PKG_VERSION"));
            ExitCode::SUCCESS
        }
        Some(other) => {
...
fn cmd_conform(args: &[String]) -> ExitCode {
    ...
    if flat {
        source = syntecnia_core::flat_syntax::translate_flat(&source);
    }
...
fn cmd_run(args: &[String]) -> ExitCode {
    ...
    let path = match path {
        Some(p) => p,
        None => {
            eprintln!("uso: syntecnia-cli run [--flat] <archivo.syn>");
            return ExitCode::from(2);
        }
    };
    let mut source = match std::fs::read_to_string(&path) {
...
fn cmd_check(args: &[String]) -> ExitCode {
    let path = match args.get(2) {
        Some(p) => p.clone(),
        None => {
            eprintln!("uso: syntecnia-cli check <archivo.syn>");
            return ExitCode::from(2);
        }
    };
    let source = match std::fs::read_to_string(&path) {
    ...
    use syntecnia_core::parser::CompileError;
    match syntecnia_core::parser::parse_source(&source, &path) {
        Ok(program) => {
...
fn cmd_tokens(args: &[String]) -> ExitCode {
    let path = match args.get(2) {
        Some(p) => p.clone(),
        None => {
            eprintln!("uso: syntecnia-cli tokens <archivo.syn>");
            return ExitCode::from(2);
        }
    };
    let source = match std::fs::read_to_string(&path) {
    ...
    match syntecnia_core::lexer::Lexer::new(&source, &path).tokenize() {
        Ok(tokens) => {
            for tok in &tokens {
                println!("  {:?}", tok);
            }
...
fn cmd_ast(args: &[String]) -> ExitCode {
    let path = match args.get(2) {
        Some(p) => p.clone(),
        None => {
            eprintln!("uso: syntecnia-cli ast <archivo.syn>");
            return ExitCode::from(2);
        }
    };
    let source = match std::fs::read_to_string(&path) {
...
fn cmd_daemon(args: &[String]) -> ExitCode {
    let action = match args.get(2).map(String::as_str) {
        Some(a) => a,
        None => {
            eprintln!("uso: syntecnia-cli daemon <start|stop|status|logs|restart> [program.syn]");
            return ExitCode::from(2);
        }
    };
    ...
    let target = match args.get(3) {
        Some(t) => t.clone(),
        None => {
            eprintln!("uso: syntecnia-cli daemon {} <program.syn>", action);
            return ExitCode::from(2);
        }
    };
