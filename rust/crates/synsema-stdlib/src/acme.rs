...
//! - `SYNSEMA_ACME_DIRECTORY` — URL del directorio ACME (default LE producción).
...
pub fn provision_certificate(
    ...
    let directory_url = std::env::var("SYNSEMA_ACME_DIRECTORY")
        .unwrap_or_else(|_| instant_acme::LetsEncrypt::Production.url().to_owned());
    let ca_root = std::env::var("SYNSEMA_ACME_CA").ok();
