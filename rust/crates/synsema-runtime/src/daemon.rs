...
//! `synsema/runtime/daemon.py`.
//!
//! ```text
//! synsema daemon start program.syn    arranca en background
//! synsema daemon stop   program.syn   detiene
//! synsema daemon status               lista todos
//! synsema daemon logs   program.syn   muestra logs
//! synsema daemon restart program.syn  reinicia
//! ```
//!
...
//! `~/.synsema/daemons/<name>/{pid,log,meta}`
...
fn daemon_dir() -> PathBuf {
    let d = home().join(".synsema").join("daemons");
...
pub fn format_status_table(statuses: &[DaemonInfo]) -> String {
    ...
    let mut lines = vec!["Synsema Daemons:".to_string()];
    for s in statuses {
