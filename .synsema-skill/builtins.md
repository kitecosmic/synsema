...
- The key is always a `secret` (text→hex, bytes→raw); never a plain string. Errors describe size/shape, never the key. Signing DENIED inside `sandbox`; ALL the encoding/digest builtins above are pure and work everywhere.
