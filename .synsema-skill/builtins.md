...
- WebSocket client (transport, gated by `net(host)` — same as HTTP): `ws_connect(url, headers?, opts?)` → handle (`ws://`/`wss://`; `opts` = `{timeout, max_message_size}`); `ws_send(conn, text_or_bytes)`; `ws_recv(conn, timeout?)` → `{type, data}` or **`nothing`** on timeout (never blocks); `ws_close(conn)`. See stdlib.md § WebSocket.
