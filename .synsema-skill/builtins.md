...
## Core
- `print(values...)` — output text. Under `synsema run` each line is written **immediately** (v0.6.29+; before, it was held until the end), and `log`/`show` too, in program order with `print`; `test`/`serve`/`conform`/`--format json` still collect it. Inside a list or map, text is shown **quoted** (v0.6.29+): `print(["1", 1, {"a": "b"}])` → `["1", 1, {a: "b"}]` (keys stay bare); a top-level `print("x")` is unchanged (`x`). `text(list)` renders the same way.
- `length(collection)` → number
- `text(value)` → string conversion (integers show no decimal: `text(42)` → `"42"`)
- `int(x)` / `int(x, fallback = f)` → **exact integer** (v0.6.29+). Accepts an integer; text with optional sign and `_` between digits (`int("-42")`, `int("1_000")`); `0x…`/`0o…`/`0b…` text, `_` allowed after the prefix (`int("0o17")` → `15`, `int("0x_ff")` → `255`; leading zeros fine: `int("0x0000…1f18")` → `7960`); a signed prefix works like Python (`int("-0x10")` → `-16`); a float/decimal whose value is whole (`int(3.0)` → `3`). Big values stay exact (`int("123456789012345678901")`). **Errors:** `int(1.5)` → `not a whole number — round it on purpose: floor/round/trunc`; `int("1.5")` → `Cannot convert`; bytes → use `bytes_to_int`. **The fallback goes by name**: `int("abc", fallback = 7)` → `7` instead of raising (`int(x, nothing)` also works — any 2nd positional that cannot be a base). **The base goes by name**: `int("ff", base = 16)` → `255`, `int("zz", base = 36)` → `1295` (base 2–36; the prefix of that base is accepted: `int("0xff", base = 16)`, `int("0o17", base = 8)`); both together: `int("zz", fallback = 0, base = 16)` → `0`. Naming both is the design, not a quirk: `int(text, 16)` would read as "base 16" to a Python reader and as "fallback 16" to the engine, so **an integer 2..36 as the 2nd positional argument is an error** that names both readings — `int("abc", 10)` → `for base 10 write int(x, base = 10); for a fallback of 10, name it: int(x, fallback = 10)` — whatever the first argument is (`int(nothing, 10)` errors too), so the result never depends on the data. Outside 2..36 a positional is still a fallback (`int("x", 100)` → `100`). A text of more than **4300 digits** → error (Python's limit; the conversion is quadratic) — `int(x, fallback = nothing)` to validate untrusted input.
- `number(value)` → numeric conversion (always float: `number("42")` → `42.0`; `_` between digits is fine: `number("1_000.5")` → `1000.5`). An integer text beyond ±2^53 → **error naming `int`** (v0.6.29+; it used to round silently) — use `int(x)`; `number(x, default)` returns the default in that case.
...
- `append(list, item)` → new list with item added — `set xs to append(xs, x)` (and `set xs to xs + [...]`) is **O(1) amortized** when nobody else holds the list (v0.6.29+: 100k appends went from minutes to under a second). `set P to insert(P, i, v)` / `merge(P, m)` also update in place (no copy), and all of them work the same when `P` is a path: `set s.items to append(s.items, x)`, `set xs[0] to append(xs[0], x)`; `set m[k] to v` in a loop is linear too. Lists/maps have value semantics, so a copy someone kept is never changed — see [types.md](types.md) § Values.
- `insert(list, i, item)` → a NEW list with `item` at position `i` (v0.6.29+): `insert([1, 2, 3], 1, 9)` → `[1, 9, 2, 3]`; `i` goes from `0` to `length` (`insert([1, 2], 2, 3)` → `[1, 2, 3]`), negatives count from the end like Python (`insert([1], -1, 0)` → `[0, 1]`). Out of range → error (`position 5 out of range for a list of length 1 (valid: -1..1)`) — Python clamps, Synsema does not.
...
- `steps()` → number (v0.6.20+) — statements the interpreter executed so far in this program. Counts nodes, not time: the same program gives the same number every run (a deterministic cost for metering/tests/fuel-style limits). No capability (introspection, like `llm_usage()`), every profile. `run --format json` reports the total as `steps`. One step per node **evaluated**, whatever internal path the engine takes (v0.6.31 fixed two statements that miscounted: `set x to x + 1` counted one step too many and `set xs to append(xs, v)` one too few)
- `join(list, separator)` → text. With **2 arguments** it joins TEXT (numbers/bools are written as text: `join(["a", 1, true], "-")` → `"a-1-true"`; an item that is `nothing`, a list, a map, bytes or a task → error, like `+` with text: `join: item 2 is nothing — convert it on purpose (text(x)), or drop the missing ones first: where(xs, (x) => x != nothing)`); with 3–4 (`join(left, right, on, how?)`, v0.6.29+) it joins **tables** — see § Tables
- `range(end)` or `range(start, end)` or `range(start, end, step)` → list. Every argument must be an integer (`2.0` is fine): `range(0, 2.5)` / `range(0, 1, 0.25)` → error `… must be an integer, got …` (v0.6.29+). `each i in range(…)` walks it without building the list (v0.6.31+: constant memory, `range(0, 5000000)` no longer takes 300 MB), and a step that would pass the largest or smallest integer ends the range, as in Python (it used to wrap around and never end)
...
## Strings
- `fmt(template, map)` → interpolated text: `fmt("Hi {name}", {"name": "Alice"})` → `"Hi Alice"`. **Strict** (v0.6.29+): a `{name}` with no value in the map → error `fmt: no value for {a} — pass it in the map, or write {{a}} for a literal brace`; `{{` / `}}` are literal braces; braces that do not surround a name stay literal (JSON/CSS pass through: `fmt("{a} {\"k\": 1}", {"a": 1})` → `1 {"k": 1}`). A hole may walk into maps (v0.6.29+): `{a.b}` is the key `"a.b"` if the map has it, else the field `b` of the map `a` — `fmt("{a.b}", {"a": {"b": 1}})` → `"1"`; `{ c }` with spaces, and `{obj.prop}` when the map has no `obj`, stay literal (CSS/JS templates pass through). Text between braces that is EXACTLY a key of the map is always replaced, whatever its shape (`fmt("[{0}] {a-b}", {"0": "z", "a-b": 1})` → `"[z] 1"`); otherwise a hole is a name starting with a letter or `_` (dots between names), so regex quantifiers stay literal: `fmt("^[0-9]{3}-{n}$", {"n": 1})` → `"^[0-9]{3}-1$"`; `{3,5}`, `{}` and `{a..b}` too
...
- `text + x`: text + number/bool concatenates (`"n=" + 1` → `"n=1"`); text + `nothing`/list/map/bytes/task → **error** (v0.6.29+) `Cannot add text and nothing — convert it on purpose: text(x), or interpolate it`
...
| `number` · `decimal` · `float` · `int` | `number(value, default)` / `int(value, fallback = default)` (by name: a positional 2..36 would read as a base) |
...
- `json_encode(value)` → text: serialize any value to a JSON string. Maps/lists nest; **secret → `"[redacted]"`** (safe), `bytes` → base64 string, `decimal` (`1.50d`) → exact JSON number, `nothing` → `null`, `nan`/`inf`/`-inf` → `NaN`/`Infinity`/`-Infinity` (Python's form; `json_decode(text, allow_nan = true)` reads them back). ⚠️ NOT safe to embed inside a `<script>` tag — use `json_for_script` there.
...
- `json_decode(text)` → value: parse a JSON string to a Synsema value (object→map, array→list, number→number, etc.). Integers of **any size stay exact** (v0.6.29+: `{"id": 12345678901234567890}`, a uint256 — they used to become floats) up to **4300 digits** (more → error `integer with 4301 digits; the limit is 4300`); a number with `.` or an exponent is a float, and one out of float range is an **error**, never `inf` (`json_decode("1e400")` → `… out of range for a float (JSON has no infinity; pass it as text)`). The bare tokens `NaN`, `Infinity` and `-Infinity` — what `json_encode` writes for `nan`/`inf` — are **not JSON** and an error by default (a NaN amount passes every `amount <= 0` check): `json_decode("[NaN]")` → `NaN/Infinity is not JSON … — to read them on purpose: json_decode(text, allow_nan = true)`; with `allow_nan = true` they read back as `nan`/`inf`/`-inf` (`json_decode("[NaN, Infinity, -Infinity]", allow_nan = true)` → `[nan, inf, -inf]`; `jsonl_decode` takes it too). A UTF-8 BOM at the start is skipped (`json_decode("\u{feff}{\"a\": 1}")` → `{a: 1}`; `jsonl_decode` too). Nesting deeper than 128 levels → error; a repeated key → the last one wins. The same parser reads `json of r` (an HTTP response), `jsonl_decode` and a `serve` request's JSON body (a uint256 in a POST body arrives exact). Errors clearly on invalid JSON; `json_decode(text, default)` returns `default` instead (see **Total variants** above).
...
- `csv_parse(text, opts?)` → list of maps (first row = headers; the same shape `sql()` returns). RFC 4180 (quoted fields, `""` escapes, CRLF/LF, BOM). **An empty field is `nothing`** (missing data — v0.6.29+; it used to be `""`), always, typed or not; a **quoted** empty field `""` is the empty text `""` (also in a `"text"`-typed column). A blank line is always skipped, also in a one-column CSV (like Python `csv` and pandas): `csv_parse("a\nx\n\ny\n")` → `[{a: "x"}, {a: "y"}]`; a blank line inside a quoted field is part of the field. Otherwise the default is **lossless text** (`"00123"` stays `"00123"`). Opts:
  ...
  - `{"headers": false}` → list of lists; `{"delimiter": ";"}` (not the quote or a line end).
  - **`{"missing": ["NA", "NULL", "-"]}`** (like polars `null_values` / pandas `na_values`): those texts, **unquoted**, are `nothing` — typed or not; a quoted `"NA"` stays the text `"NA"` (someone wrote it on purpose). `csv_parse("a,b\nNA,1\n\"NA\",2\n", {"missing": ["NA"]})` → `[{a: nothing, b: "1"}, {a: "NA", b: "2"}]`.
  - A `float` column reads `nan`/`inf` written as such, and `1e400` is an **error** (`out of range for a float`), never infinity; an `int` column (or `numbers: true`) with more than 4300 digits is an error (the same limit as `int()`); `decimal` takes any number of digits.
  - Errors carry the line (unclosed quote, uneven fields, duplicate headers, unknown option).
  - `csv_parse("a,b,c,d\n007,,2026-01-02,1.50\n", {"types": {"a": "int", "c": "date", "d": "decimal"}})` → `[{a: 7, b: nothing, c: 2026-01-02, d: 1.50}]`; untyped: `csv_parse("a,b\n007,\n")` → `[{a: "007", b: nothing}]`; `csv_parse("a,b\n\"\",\n")` → `[{a: "", b: nothing}]`.
- `csv_encode(value, opts?)` → text. List of maps (headers = first map's keys) or list of lists. Opts: `{headers: [..]}` (order/subset), `{delimiter}`, `{eol}` (`"\r\n"` default). `{"escape_formulas": true}` — OWASP "CSV injection": a **text** cell that starts with `=`, `+`, `-`, `@`, tab or CR gets a leading `'` so Excel/Sheets show it instead of running it (`=SUM(A1)` → `'=SUM(A1)`; numbers like `-5` are untouched). Off by default, because it changes the data; turn it on for any CSV a person opens in a spreadsheet and whose text came from users. Minimal quoting; integers without decimals; `nothing` → an empty field and the empty text → `""` (so `csv_parse(csv_encode(rows)) == rows` for both: `csv_encode([{"a": "", "b": nothing}])` → `a,b` / `"",`). The one exception: a row whose only field is `nothing` is written `""` (an empty line would be skipped on reading, as in Python's writer), so in a one-column CSV a missing value comes back as the empty text — `csv_encode([{"a": nothing}])` → `a` / `""` — or `nothing` if you read the column with a type (it warns once on stderr: `csv_encode: a one-column table writes nothing as "" …`). `{"missing": "NA"}` (pandas' `na_rep`) writes each `nothing` as that mark, unquoted, and quotes a text equal to it, so `csv_parse(csv_encode(rows, {"missing": "NA"}), {"missing": ["NA"]}) == rows` with any number of columns: `csv_encode([{"a": nothing}, {"a": "NA"}], {"missing": "NA"})` → `a` / `NA` / `"NA"`. The mark must be non-empty text (an empty field already reads as `nothing`) without the delimiter, a quote or a line end; dates/datetimes/durations → their ISO text; `bytes` → base64; **secret → `[redacted]`**; nested list/map → error suggesting `json_encode`.
...
- `parquet_read(bytes, opts?)` → rows (list of maps), typed: Int64 → exact int, Double/Float/Float16 → float, Decimal → `decimal` (one of more than 28 significant digits does not fit a `decimal`: with a fractional part it comes back as its exact **text** in decimal notation, `"1234567890123456789012345678901.4567890"`; without one, as an exact integer), String → text, Binary → `bytes`, Date → `date`, Timestamp → `datetime` (UTC, or the column's zone: the one `parquet_write` stored, else the one polars/pyarrow wrote in the Arrow schema — `datetime[μs, America/Sao_Paulo]` comes back as `2026-01-03T10:00:00-03:00[America/Sao_Paulo]`; a fixed offset like `+05:30` (pyarrow, pandas) keeps it: `2026-01-03T14:30:00+05:30`, the same instant and the same local time; ms/µs/**ns** — pandas/polars nanosecond columns read as datetimes with their nanoseconds), Time (time of day, ms/µs/ns) → a `duration` since midnight (there is no time-of-day type: `in_units(r.t, "seconds")` → `47445.123456` for 13:10:45.123456), lists/groups → nested lists/maps, null → `nothing`. Reads zstd / snappy / gzip / lz4 (verified with files written by polars 1.44). Not a Parquet file → error `not a Parquet file`. Two columns with the same name → error (`the file has two columns named "a" — a row is a map and would lose one`).
  - **Expansion caps** (v0.6.29+): a small file cannot make the engine allocate gigabytes. Checked **before** anything is allocated: a page that declares more than **256 MiB** uncompressed (`a page declares 2047 MiB uncompressed (the limit is 256 MiB) — refusing to allocate it`), a file that expands past **`max_bytes`** (default the larger of 1 GiB and 64 × the file's size), more than **`max_cells`** = rows × columns (default 50 000 000: `the file has 2000 cells (rows × columns); the limit is 100 — raise it on purpose with {"max_cells": n}`). Raise them on purpose for a big trusted file: `parquet_read(b, {"max_cells": 200000000, "max_bytes": 8589934592})`; an unknown option → error.
- `parquet_write(rows, opts?)` → bytes. The schema is **inferred per column**, every column OPTIONAL (`nothing` = null): int → INT64, a number with decimals → DOUBLE (a column mixing int and float → DOUBLE; an integer beyond 2^53 there is an **error** naming the column and row, since a DOUBLE would round it: `parquet_write: column "a", row 2: the integer 9007199254740993 does not fit a float exactly, … — convert the column to decimal or to text`), `decimal` → DECIMAL(38, the column's max scale), text → STRING, bool → BOOLEAN, `bytes` → BINARY, `date` → DATE, `duration` → INT64 in µs (ns when a value needs them), as Arrow stores it, with its unit in the file metadata (`synsema.durations`), `datetime` → TIMESTAMP(µs, UTC) — or TIMESTAMP(**ns**) when a value of the column has nanoseconds, so it round-trips exactly (`datetime("2026-01-03T10:00:00.123456789Z")` comes back equal; a column with nanoseconds AND a value outside 1677-2262 — or a duration beyond ±292 years — is an **error** naming the column, since one unit cannot hold both: split it or store it as text) — with the column's zone (IANA or a fixed offset like `+05:30`) in the file metadata (`synsema.timezones`), so `parquet_read(parquet_write([{"when": datetime("2026-01-03T10:00:00", "Europe/Madrid")}]))` gives back `2026-01-03T10:00:00+01:00[Europe/Madrid]` (other readers see UTC). `opts.compression` = `"snappy"` (default) | `"zstd"` | `"gzip"` | `"lz4"` | `"none"`. **Errors:** a column that mixes types (`[{"a": 1}, {"a": "x"}]` → `column "a" mixes Int and Text — one type per column`), a nested list/map value (→ `json_encode` that column first), an int beyond 64 bits. The file carries the **Arrow schema** (`ARROW:schema`, as pyarrow, polars and arrow-rs write it), so pandas, pyarrow and polars read every column with its type: a `duration` as `timedelta64`/`Duration`, a `datetime` with its IANA zone (`datetime64[us, Europe/Madrid]`). A fixed-offset zone (`+05:30`) goes as UTC in that schema, because polars cannot open a file with one; Synsema reads the offset back from its own metadata. polars reads what it writes back with the exact types.
```synsema
...
- floor division: the operator `a // b` (v0.6.29+) — exact for integers of any size, floors like Python (`-7 // 2` → `-4`, `7.5 // 2` → `3.0`); on floats `//` and `%` give **exactly CPython's (and numpy's) answer**, computed from the real binary values: `7 // 0.1` → `69.0`, `7 % 0.1` → `0.09999999999999962` (0.1 is stored as a bit more than 0.1); arrays too (`array([7.0]) // 0.1` → `[69]`). See [syntax.md](syntax.md). `a / b` is always float.
...
- **descriptive statistics** (see [dataviz.md](dataviz.md)): `median(x)`; `percentile(x, p)` with **p ∈ [0, 100]**; `quantile(x, q)` with **q ∈ [0, 1]** (v0.6.29+; `quantile([1], 50)` → error `… between 0 and 1`) — both linear interpolation, the NumPy default, both take `axis =` on arrays; `percentile(x, 50) == quantile(x, 0.5) == median(x)`. `synsema check` warns on `percentile(x, 0.5)` (a literal p in (0, 1) — "for a fraction use quantile"). `histogram(x, bins?)` → `{counts, edges}` (`bins` = int, default 10, or explicit ascending edges; last bin closed). `mode(xs)` → the most frequent value (any type, counted by `==` like the group keys of § Tables; first seen wins a tie; `nothing` skipped; empty → error; linear time). Correlation, covariance and fits: § Statistics and fitting.
...
5. **Decimals stay decimal**: `mean([1.5d, 2.5d])` → `2.0` (decimal), `median([1d, 2d, 4d, 8d])` → `3` (decimal), `sum([0.1d, 0.2d])` → `0.3` exactly. `median`/`var`/`std`/`quantile`/`percentile` of decimals — integers of any size included — are computed **exactly**, like Python's `statistics`: `var([1000000000000000000001d, 1000000000000000000003d])` → `2` (a float would give `0`), `median([1d, 2d, 10**30])` → `2`, `std([1d, 2d])` → `0.7071067811865475244008443621`, and `percentile(xs, 90)` uses exactly `0.9` (`percentile([1d, 2d, 4d], 90)` → `3.6`, a decimal). The result is **always a decimal**: exact when it terminates, otherwise 28 significant digits (`std` of decimals that differ by 1e-15 is `0.000000000000001527525231651946668862682398`, never `0` and never a float). Decimals have arbitrary precision: `sum([1d, 10**30])` → `1000000000000000000000000000001` (a decimal), `1d < 10**30` → `true`.
...
- **Locate and accumulate** (v0.6.29+, lists or arrays): `argmin(x, axis = k)` / `argmax(x, axis = k)` → position of the extreme (first on a tie; a NaN wins, like numpy — the first NaN if any). On a list `nothing` is skipped and the index is still the position in the original list (`argmin([nothing, 3, 1])` → `2`); every value `nothing` → error. `argmax([3, 9, 2])` → `1`, `argmax(array([[1,5],[7,2]]), axis = 1)` → `[1, 0]`. `cumsum(x)` → running sums: a list stays exact (ints/decimals stay exact, a `nothing` stays `nothing` in its place), an array is flattened unless you pass `axis =` (`cumsum([1,2,3])` → `[1, 3, 6]`). `diff(x)` → consecutive differences (`diff([1,4,9])` → `[3, 5]`); on an array along the LAST axis by default (numpy), or `axis =`.
...
- `lstsq(A, b)` → least-squares solution `x` of `A·x ≈ b` — **numpy's `np.linalg.lstsq`** (SVD with numpy's default `rcond`): a rank-deficient `A` or fewer rows than columns gives the **minimum-norm** solution, like numpy (`lstsq(array([[1,1],[2,2]]), array([2,4]))` → `[0.9999999999999998, 1]`; `lstsq(array([[1,2]]), array([5]))` → `[1.0000000000000004, 2]`). A 2-D `A`; a 1-D `b` gives a 1-D `x`: `lstsq(array([[1,0],[0,1],[1,1]]), array([1,2,3]))` → `[0.9999999999999997, 2]` (float round-off, as in numpy — compare with a tolerance).
- `polyfit(xs, ys, degree)` → coefficients, **highest degree first** (numpy order, the columns scaled like numpy): `polyfit(x, y, 1)` = `[slope, intercept]` (`polyfit([0,1,2,3], [1,3,5,7], 1)` → `[1.9999999999999998, 1.0000000000000004]`). A fit the data does not determine (fewer distinct x than `degree + 1`) is an **error** (`polyfit([1,1,1], [1,2,3], 1)` → `… not determined by the data (rank 1 of 2) …`) — numpy only emits a `RankWarning`. `polyval(coefs, x)` evaluates them on a number, a list or an array (`polyval([2.0, 1.0], 10)` → `21.0`); a missing `x` stays `nothing` in its place, like `cumsum` (`polyval([2.0, 1.0], [1, nothing, 3])` → `[3.0, nothing, 7.0]`), and a missing coefficient is an error (→ `drop_missing` first).
...
- `lineage()` → list of `{source, what, sha256, bytes, encoding}` so far, plus `salt` and `committed_encoding` on a query or prompt entry (local only — `receipt()` never publishes them). **Revealing a query** later: give the verifier the arguments and the salt; they check `"query sha256-salted:" + slice(hex(sha256(bytes(salt, "hex") + bytes(canonical_json(args), "utf8"))), 2) == what` (`args` = the call's arguments as a list, e.g. `[q, [5]]` for `sql(q, [5])`; `committed_encoding` is `"jcs"`, or `"json"` → `json_encode(args)` when they hold an integer beyond 2^53). `sha256` = hex, **no `0x`**, of the bytes the program received; `bytes` = their length; `encoding` (v0.6.29+) says **which bytes** were hashed, so anyone can recompute it:

| `encoding` | the hash covers | recompute it with |
|---|---|---|
| `"text"` | the UTF-8 of the text received | `hex(sha256(t))` |
| `"bytes"` | the raw bytes | `hex(sha256(b))` |
| `"jcs"` | `canonical_json(x)` of the value received (RFC 8785) | `hex(sha256(canonical_json(x)))` |
| `"json"` | `json_encode(x)` — used when the value holds an integer beyond 2^53, which JCS cannot carry | `hex(sha256(json_encode(x)))` |

So `"0x" + e.sha256 == hex(sha256(json_encode(rows)))` when `e.encoding == "json"` (e.g. a SQL row with `9007199254740993`); the receipt's `credentialSubject.inputs` carry the same field. What each builtin records:
...
| `list_dir`, `file_info`, `grep`, `parquet_read` | the path as passed (`grep`: the target, never the pattern); a file passed as bytes (`parquet_read(b)`) is `bytes <n>` — its size, never its content | the result the program received |
| `http_get`, `http_post`, `http_put`, `http_delete`, `http`, `http_bytes`, `fetch` | the **HOST only** (with the port when the URL has one: `127.0.0.1:8080`) — never `user:pass@`, the path or the query (an API key in the URL is never recorded); a 4xx/5xx answer is an input too, with ` status N` appended (`api.example.com status 404`) | the response body the program received (`http(method, url, …)` records the host of `url`) |
| `sql`, `sql_batch`, `sql_tables`, `paged`, `mongo_find`, `mongo_find_one`, `mongo_aggregate`, `mongo_count`, `mongo_collections`, redis reads (`redis_get`, `redis_mget`, `redis_hget`, `redis_hgetall`, `redis_lrange`, `redis_smembers`, `redis_sismember`, `redis_keys`, `redis_exists`, `redis_llen`, `redis_lpop`, `redis_rpop`, `redis_ttl`, `redis_type`) and counters (`redis_incr`, `redis_incrby`, `redis_decr`, `redis_hincrby`), `recall` | `query sha256-salted:<hex>` — a **salted commitment** to the whole query (all its arguments, Mongo filters included), like an SD-JWT disclosure: `sha256(salt ‖ canonical_json([args…]))` with a fresh 128-bit salt per entry. The receipt never carries the query or the salt, so nobody recovers the query by hashing guesses (a plain sha256 of `run("id", "-u")` could be); the salt stays in the local `lineage()` so the owner can reveal it later (see below) | the result as `canonical_json(result)` (RFC 8785; `encoding` `"jcs"`) — recompute it with `sha256(canonical_json(rows))`; with an integer beyond 2^53 in it, `json_encode(result)` (`encoding` `"json"`) |
| `run`, `run_program` (what an external command / a child program returned — v0.6.29+) | `query sha256-salted:<hex>` of the arguments | the result map (`jcs`) |
| Chain readers with a URL: EVM (`evm_rpc`, `evm_call`, `evm_balance`, `evm_nonce`, `evm_chain_id`, `evm_gas_price`, `evm_estimate_gas`, `evm_fee_history`, `evm_block_number`, `evm_logs`, `evm_receipt`, `evm_wait`), Solana (`solana_rpc`, `solana_balance`, `solana_latest_blockhash`, `solana_wait`), Algorand (`algorand_account`, `algorand_params`, `algorand_wait`), Bitcoin (`btc_rpc`, `btc_balance`, `btc_utxos`, `btc_fee_estimates`, `btc_wait`) | `<host> query sha256-salted:<hex>` — the host (same rule as HTTP; only from an argument that IS a URL, `scheme://…` with no spaces — a `://` inside a SQL query or a command is data and is never published) + the salted commitment to the arguments | the answer |
| Live feeds: `ws_recv`, `ws_select`, `ws_select_all`, `proc_recv`, `proc_select`, `proc_wait`, `term_recv`, `watch_recv` | `query sha256-salted:<hex>` of the arguments | the message (a receive that timed out with nothing is not an input) |
| `reason`, `decide`, `analyze`, `generate` (a real model's answer) | source `llm`, what `<kind> prompt sha256-salted:<hex>` (`reason prompt sha256-salted:…`, the same salted commitment over the prompt text) | the text the model returned |
| `read_line` | `stdin` | the line read |

`what` is published in the receipt, so it only ever holds a path the program passed as text, a host, a salted commitment or a size — never data. Nothing else is recorded: not what the program writes, not `env`/`secret`, not the blackboard, not an offline LLM placeholder. Only reads that arrived count (an HTTP call that failed on the network is not an input). The list stops at 10,000 entries (the receipt says `inputs_truncated: true`).
...
- `unique(list)` → deduplicated, first-appearance order (structural equality, same as `==`/`contains` — maps/lists dedupe by value; `unique([1, 1.0, true, "1"])` → `[1, "1"]`; linear time since v0.6.29)
...
**Keys** (`group_by`, `summarize`, `count_by`): a column name (text), a list of column names (the key becomes a map `{col: value, …}`), or a function `(row) => …`. A group's key keeps its **value and type** and groups by `==` (the same key in `group_by`/`count_by`/`summarize`/`pivot`/`unique`/`mode`/`n_unique_of`): `1`, `1.0` and `true` are one group, the text `"1"` another; `{"a": 1, "b": 2}` and `{"b": 2, "a": 1}` are one group; the same instant in two zones is one group; `-0.0` and `0.0` are one group, also inside arrays (`array([-0.0])` and `array([0.0])`); every NaN falls in ONE group (like polars/numpy). Groups come in **first-appearance order**. A column name that appears in **no** row is an error (`group_by: no row has a column "G" (columns: "g", "v")`) — the same for `sum_of("vv")` & co., `join` and `pivot`; a column present in only some rows is `nothing` where it is absent.
...
- `summarize(rows, by, aggs)` → rows: the key column(s) + one column per aggregate. `aggs` = map `name → aggregate`. With `by` = a column, that column keeps its name; with a list, each key column; with a function, the key goes in a column `key`. An aggregate named like a key column → error (`the aggregate "g" has the name of a key column — it would overwrite the key`). The column of an aggregate is checked against the **whole table**, not group by group: with uneven rows (`[{"g": "a", "v": 1}, {"g": "c"}]`) the group `c` just has no values (`sum_of("v")` → `0`, `mean_of("v")` → `nothing`); only a column that no row of the table has is an error. Called on its own (`sum_of("v")(rows)`), an aggregate checks the list you give it. Built-in aggregates (each returns a function of the group):
...
- `join(left, right, on, how?)` → rows (a hash join — linear). `on` = a column or a list of columns present on both sides; `how` = `"inner"` (default) | `"left"` | `"right"` | `"outer"` | `"semi"` | `"anti"`. A non-key column present on both sides: the right one gets the suffix **`_right`** (polars). If that name is already taken it is an **error**, never an overwrite (like polars): the left side already has `b_right` → `join: the right column "b" would be named "b_right", which the left side already has — rename one of them first`; two right columns landing on one name → `the right columns "b" and "b_right" would both be named "b_right"`. `semi`/`anti` add no right columns, so they never raise these (`join([{"k": 1, "x": 1, "x_right": 0}], [{"k": 1, "x": 3}], "k", "semi")` → `[{k: 1, x: 1, x_right: 0}]`). **Every output row has every column**: a row with no match gets `nothing` in the other side's columns (in `outer`/`right`, a right-only row carries the right key and the left columns as `nothing`); a `nothing` key never matches (SQL). Order: the left rows, then (`right`/`outer`) the unmatched right rows — an `outer` join with an empty left keeps the key (`join([], [{"k": 1, "b": 1}], "k", "outer")` → `[{k: 1, b: 1}]`). `semi` → the left rows that have a match, `anti` → those that don't, unchanged (no right columns). With `L = [{"id": 1, "a": "x"}, {"id": 2, "a": "y"}]`, `R = [{"id": 1, "b": "p"}, {"id": 3, "b": "q"}]`: `join(L, R, "id", "right")` → `[{id: 1, a: "x", b: "p"}, {id: 3, a: nothing, b: "q"}]`, `join(L, R, "id", "anti")` → `[{id: 2, a: "y"}]`. `join([{"id": 1, "a": "x"}, {"id": 2, "a": "y"}], [{"id": 1, "a": "z"}], "id", "left")` → `[{id: 1, a: "x", a_right: "z"}, {id: 2, a: "y", a_right: nothing}]`. ⚠️ **Arity decides**: `join(items, sep)` with 2 arguments is still the TEXT join (`join(["a", "b"], "-")` → `"a-b"`).
- `pivot(rows, index, columns, values, agg?)` → one row per value of `index`, one column per distinct value of `columns` (in first-appearance order), cell = `values`. `index`/`columns`/`values` are column names (text). A cell that several rows fall into needs `agg` (an aggregate, e.g. `sum_of("v")`, or any `(group) => …`) — without it that is an **error** (`… rows fall in the same cell … — pass agg, e.g. sum_of("v")`), never a silent "first". A missing cell → `nothing`. Two `columns` values that print the same (`1` and `"1"`), or one named like the `index` column → error (`the values 1 and "1" of "c" would both become the column "1"`) — convert the column first. `pivot([{"d": "lu", "p": "a", "v": 1}, {"d": "lu", "p": "b", "v": 2}, {"d": "ma", "p": "a", "v": 3}], "d", "p", "v")` → `[{d: "lu", a: 1, b: 2}, {d: "ma", a: 3, b: nothing}]`.
- **Missing data** (`nothing` = missing; NaN = invalid number — different things):
  - `is_missing(x)` → `x == nothing`.
  - `count(xs)` → how many values are **present** (not `nothing`; pandas/SQL `count`): `count([1, nothing, 3])` → `2`. `count_missing(xs)` → how many are `nothing`. Lists only; `count()` with no argument is still the `summarize` aggregate (rows in the group).
...
Three types (see [types.md](types.md) § Dates): **`date`** (a civil day, no time or zone), **`datetime`** (an instant + its zone: an IANA zone, DST-aware, or a fixed offset like `+05:30`), **`duration`** (an exact amount of time, nanoseconds). Building, parsing, formatting, comparing and computing are **pure** — only `now()` and `sleep()` read the clock and need `require time`.
...
- `datetime("2026-01-03T10:00:00Z")`, `datetime("2026-01-03T10:00:00-03:00")` (an offset **keeps its offset**, like Python, java.time and Temporal: it prints `2026-01-03T10:00:00-03:00` and `date_parts` gives hour 10; `+0530` without the colon reads too; `Z` and `+00:00` are UTC), `datetime("2026-03-29T01:30:00", "Europe/Madrid")` (local time + zone), `datetime("2026-01-03")` (midnight UTC), `datetime("2026-01-03T10:00:00[Europe/Madrid]")` (RFC 9557: without an offset it is the **local time in that zone**; with one — what it prints, `…T10:00:00+01:00[Europe/Madrid]` — the offset must match the zone at that instant, or it is an error, as in Temporal), `datetime(text, zone)` with a text that has an offset → the same instant **converted** to `zone`, `datetime(2026, 1, 3, 10, 0, 0, "UTC")` (hour/minute/second optional, zone last, default UTC), `datetime(timestamp_seconds, tz?)`, `datetime(date, tz)` (midnight in `tz`). A local time inside a **DST gap** → error `… does not exist in Europe/Madrid (it falls in a daylight-saving gap)`; an ambiguous one (the hour that repeats when clocks go back) takes the **first** (java.time / Temporal "compatible"). A zone is an IANA name (`"America/Buenos_Aires"`, `"Europe/Madrid"`), `"UTC"`, or a fixed offset (`"+05:30"`, `"-0300"`, `"+05"`) — everywhere a zone goes (`datetime`, `to_timezone`, `parse_datetime`); an unknown one → error suggesting both. A fixed offset has no daylight saving: for a city, use its IANA zone.
...
- `add_days(t, n)` → calendar days: a datetime keeps its **local time** across a DST change (`add_days(datetime("2026-03-28T12:00:00", "Europe/Madrid"), 1)` → `2026-03-29T12:00:00+02:00[…]`, while `+ duration(days = 1)` is 24 h → `13:00`); on a date it is `d + duration(days = n)`. When the calendar step lands in a DST **gap**, `add_days`/`add_months` (and "start of the day") move forward by the gap — Temporal's "compatible" rule: `add_days(datetime("2026-03-28T02:30:00", "Europe/Madrid"), 1)` → `2026-03-29T03:30:00+02:00[…]`. Building or parsing that nonexistent time directly is still an error.
...
- `truncate(t, unit)` → the start of the period: `"year"`, `"quarter"`, `"month"`, `"week"` (Monday), `"day"`, `"hour"`, `"minute"`, `"second"` (the last three only on a datetime; in its own zone). On a day with no midnight (a DST jump at 00:00) `"day"` gives the first instant of the day: `truncate(datetime("2024-09-08T15:00:00", "America/Santiago"), "day")` → `2024-09-08T01:00:00-03:00[America/Santiago]`; on a day whose midnight **repeats** (a fall-back at 01:00 → 00:00, as in Havana) `"day"` gives the FIRST midnight, so the day is never split in two: `truncate(datetime("2024-11-03T12:00:00", "America/Havana"), "day")` → `2024-11-03T00:00:00-04:00[America/Havana]`; `"hour"` (or finer) inside a repeated hour keeps which of the two passes it was (the offset stays: `00:30-05:00` → `00:00-05:00`). The tool for **grouping by period**: `summarize(rows, (r) => truncate(r.when, "month"), {…})`.
- `date_range(start, end, step?)` → list from `start` to `end` **inclusive**; `step` = a positive duration or `"day"` (default) | `"week"` | `"month"` | `"quarter"` | `"year"`, or on datetimes `"hour"` | `"minute"` | `"second"` (elapsed time, like a duration; on dates → error `a date has no hours`). `date_range(date(2026,1,1), date(2026,1,3))` → `[2026-01-01, 2026-01-02, 2026-01-03]`. Start and end must be the same type. On datetimes `"day"`/`"week"` step in **calendar** days, so the local time survives a DST change (Madrid, 12:00 on 03-28 → `12:00+01:00`, `12:00+02:00`, `12:00+02:00`); a `duration` step is elapsed time. A calendar step that lands on a day that never existed (Pacific/Apia skipped 2011-12-30) is skipped, not repeated: the range goes 12-29, 12-31 (`add_days` on its own still moves 12-29 + 1 to 12-31).
- `to_timezone(dt, "UTC")` → the same instant seen in another zone (IANA or a fixed offset: `to_timezone(dt, "-03:00")`). `timestamp(dt)` → float unix seconds (a datetime only; for a date: `timestamp(datetime(d, "UTC"))`). `in_units(d, unit)` → float, unit = `"weeks"` | `"days"` | `"hours"` | `"minutes"` | `"seconds"` | `"milliseconds"` (`in_units(duration(hours = 1, minutes = 30), "minutes")` → `90.0`).
- `parse_date(text, format?)` → date; `parse_datetime(text, format?, tz?)` → datetime (strftime: `parse_date("31/01/2026", "%d/%m/%Y")`; a format without an offset reads local time in `tz`, default UTC; a format with `%z` keeps the text's offset, or converts it to `tz` if you pass one: `parse_datetime("24/09/2026 02:00 +0530", "%d/%m/%Y %H:%M %z")` → `2026-09-24T02:00:00+05:30`; a date-only format is midnight: `parse_datetime("03/01/2026", "%d/%m/%Y")` → `2026-01-03T00:00:00Z`). `%Z` (a zone abbreviation) accepts only `UTC` and `GMT` (any case, like Python): an abbreviation does not identify a zone (`IST` is India, Ireland or Israel), so `"10:00 IST"` is an **error**, not 10:00 UTC — use `%z` for an offset, or drop the abbreviation and pass the zone; with `%z` in the same format the offset decides and the abbreviation is just text: `"+0530 (IST)"` reads with `"%z %Z"` (`%Z` takes the whole word, parentheses included; `"%z (%Z)"` does not match). A mismatch → error naming the text and the format.
- Display: date `2026-01-31`; datetime RFC 3339, `…Z` in UTC, `2026-09-24T02:00:00+05:30` with a fixed offset, and `2026-03-29T01:30:00+01:00[Europe/Madrid]` with an IANA zone (an offset with seconds — a zone's local mean time before ~1920 — keeps them, as in Python: `1900-01-01T00:00:00-04:16:48[America/Buenos_Aires]`, and reads back); duration ISO 8601 (`P29D`, `PT1H30M`). `json_encode`/`csv_encode` write that ISO text.

## Seeded randomness (pure — v0.6.29+)

- `rng(seed)` → a **generator**: a function value; `g()` returns the next uniform float in `[0, 1)`. **Bit for bit `numpy.random.default_rng(seed)`** (SeedSequence → PCG64): the same seed gives numpy's numbers, on every platform and every version. `seed` = an integer ≥ 0 of any size (like numpy); negative → error.

| Synsema | numpy (`g = np.random.default_rng(seed)`) |
|---|---|
| `g()` / `random(g)` | `g.random()` |
| `random_int(g, lo, hi)` (inclusive) | `g.integers(lo, hi + 1)` |
| `random_normal(g, mean = 0, std = 1)` | `g.normal(mean, std)` |
| `shuffle(g, xs)` → a NEW list | `g.permutation(xs)` |
| `choice(g, xs)` → one item (empty → error) | `g.choice(xs)` |
| `sample(g, xs, n)` → `n` distinct items (n > length → error) | `g.choice(xs, n, replace=False)` |
| `rng_spawn(g, n)` → a list of `n` independent generators | `g.spawn(n)` |

- `rng(42)()` → `0.7739560485559633`, the same as `np.random.default_rng(42).random()`. `mean`/`std` go by name; `std` < 0 → error.
...
- A generator is a **process, not a value** (as in numpy): `let h be g` or passing `g` to a task gives the SAME generator — `g` and `h` take turns on one sequence. For independent streams use `rng_spawn(g, n)` (each call gives new children, like numpy's `spawn`; linear — 40 000 children in a fraction of a second) or separate seeds. Any task returning a float in `[0, 1)` also works as the generator of `random_int`/`shuffle`/… (without the numpy match).
- **Generators and `parallel_map`** (v0.6.29+): a generator passed as an item reaches its worker with its state, the worker draws the numbers numpy would, and **the caller's generator advances by what the worker drew** — exactly as with `apply`, so using `g` (or the children of `rng_spawn`) again continues the sequence instead of repeating it. `parallel_map(draw, rng_spawn(g, n))` gives exactly numpy's `[c.random() for c in g.spawn(n)]`. The **same** generator in two items (at any depth) is an error (numpy silently copies it into every process, so every worker draws the same numbers): `parallel_map: the same generator reaches more than one item — each worker would draw the same numbers; give each item its own: rng_spawn(g, length(items))`. Pass generators as items or arguments.
- **A top-level generator does not cross** into a `serve` request or a `parallel_map` worker (by name, or inside a global map or list): there it would restart the same sequence every time, so using it is an error that says so — `the generator rng(1) was created at the top level; here it would restart the same sequence in every request/worker …`. Under `serve`, make the generator **inside** the request from something request-specific — `rng(int(params.seed))`, or `rng_spawn` over a per-request seed — or, when reproducibility is not the point, use `random()` with `require random` (OS entropy, different every request).
- Not for secrets or tokens — that is `random_bytes`/`token` (OS CSPRNG, `require random`).
```synsema
task draw(g)
    give g()
let per_worker be parallel_map(draw, rng_spawn(rng(42), 3))   -- numpy: [c.random() for c in default_rng(42).spawn(3)]
let g be rng(42)
...
let workers be rng_spawn(g, 4)                    -- 4 independent generators (numpy's g.spawn(4))
```
...
- `sql_exec(statement, params?)` → {rows_affected, last_id} (INSERT/UPDATE/DELETE/CREATE). On **SQLite** a statement with `RETURNING` is refused **before it runs** (`sql_exec: this statement returns rows (RETURNING) — run it with sql(statement, params) to get them (nothing was executed)`) — SQLite would write the row and fail afterwards; the word inside a string or a comment does not count. Postgres and MySQL run it and return the count (the rows are dropped). A `SELECT` errors on every engine. To get the rows back, write `sql("INSERT … RETURNING id", params)`.
  `last_id`: SQLite rowid; MySQL `last_insert_id()`; Postgres `0` (use `RETURNING`).
- `sql_batch(statement, params_list)` → {rows_affected} (batch operations). On SQLite a `RETURNING` statement is refused before ANY row runs (`sql_batch: this statement returns rows (RETURNING) — … (nothing was executed)`), like `sql_exec`.
...
- `cron_every(expr, task, opts?)` → job name (v0.6.12+) — **wall-clock cron expression** (text): 5 fields `min hour day month weekday` (`*`, `a-b`, `*/n`, lists, `jan..dec`/`sun..sat`, `0`/`7` = Sunday; day+weekday both set → OR, like Vixie cron) or an alias `@hourly`/`@daily`/`@weekly`/`@monthly`/`@yearly`. `opts = {"tz": "UTC" | "+HH:MM" | "America/Santiago"}` (default UTC; an IANA zone follows its clock changes, with the Vixie cron / cronie rule: a fixed time that falls in the skipped spring hour runs right after the jump instead of being lost; a fixed time in the repeated autumn hour runs once, the first time; a job with `*` in the minute or hour field (`*/15 * * * *`, `0 * * * *`) follows real time — skipped minutes do not exist and the repeated hour runs twice). An unknown zone → error at registration. Fires at the next matching minute AFTER the previous run ends (occurrences skipped while a run is in progress are dropped, never queued); no persistence, no catch-up. Bad expression / never-matching (`0 0 31 2 *`) / unknown option → error at registration
- `cron_after(seconds, task)` → job name (one-shot; delay ≥ 0; same validation)
- `cron_cancel(name)` → bool
- `cron_list()` → list of `{name, schedule, interval, repeating, active, run_count, errors, next_run, tz}` — `schedule` = `"every 300.0s"` / `"after 60.0s"` / the cron expression; `interval` = seconds or `nothing` (expression jobs); `next_run` = unix timestamp of the next fire (`nothing` once a one-shot ran); `tz` = `"UTC"`/`"+HH:MM"`/`"America/Santiago"` or `nothing` (interval jobs); run_count = completed runs, errors = failed ticks (real counts, never phantom)
