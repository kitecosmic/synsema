...
- `parse_date(text, format?)` → date; `parse_datetime(text, format?, tz?)` → datetime (strftime: `parse_date("31/01/2026", "%d/%m/%Y")`; a format without an offset reads local time in `tz`, default UTC; a format with `%z` keeps the text's offset, or converts it to `tz` if you pass one: `parse_datetime("24/09/2026 02:00 +0530", "%d/%m/%Y %H:%M %z")` → `2026-09-24T02:00:00+05:30`; a date-only format is midnight: `parse_datetime("03/01/2026", "%d/%m/%Y")` → `2026-01-03T00:00:00Z`). `%Z` (a zone abbreviation) accepts only `UTC` and `GMT` (any case, like Python): an abbreviation does not identify a zone (`IST` is India, Ireland or Israel), so `"10:00 IST"` is an **error**, not 10:00 UTC — use `%z` for an offset, or drop the abbreviation and pass the zone; with `%z` in the same format (`"+0530 (IST)"`, `"%z (%Z)"`) the offset decides and the abbreviation is just text. A mismatch → error naming the text and the format.
