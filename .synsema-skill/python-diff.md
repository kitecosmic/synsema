...
| `if / elif / else:` | `when / otherwise when / otherwise` (no colon) | a trailing `:` → parse error `Synsema blocks have no colon`; `elif` / `else` alone on their line → a hint with the form (`` `elif` is not a Synsema statement: chain with `otherwise when <condition>` ``) |
| `x if c else y` / `c ? a : b` | `when c then x otherwise y` | inline expression form, usable in `let`/args; `a if c else b` and `c ? a : b` → an error with this hint (v0.6.29+) |
| `a && b` / `a \|\| b` / `a; b` | `a and b` / `a or b` / one statement per line | `&`, `\|` → ``the logical operators are `and` / `or` ``; `;` → ``one statement per line (no `;`)`` (lexer errors, v0.6.29+) |
| `continue` / `break` / `pass` | wrap the rest of the body in `when` / `stop` / leave the branch out | Synsema has **no `continue`**: the hint says ``skip to the next item by wrapping the rest of the body in `when` ``; `break` → `stop`; `pass` → an empty block is not needed |
| `for x in xs:` | `each x in xs` | `for` → parse error |
| `x += 1` | `set x to x + 1` | no compound assignment: `+=` → parse error `Synsema has no compound assignment (+=): write set x to x + 1` |
...
| `x is None` | `x == nothing` / `is_missing(x)` | no `is` operator for identity (`is` belongs to `match`); the parse error says so |
| `"\u00e9"`, `"\U0001F600"`, `"\x41"`, `0o17` | `"\u00e9"`, `"\u{1F600}"`, `"A"`, `0o17` | v0.6.29+; the 8-digit `\U` is `\u{…}` (1–6 hex). **`\x` is not an escape** (`"C:\build\x64"` stays as written); an incomplete `\u` stays literal (`"C:\users"`); `_` in a number only between digits (`1__0`, `1_` → lexer error) |
| `f"n={n}"` | `` `n={n}` `` (backtick string) | `f"..."` → parse error naming the backtick template. A hole takes any value (lists, maps, `nothing`, bytes), like an f-string (v0.6.29+). **Quoted `"..."` strings do NOT interpolate** (`"{n}"` stays literal) and a literal newline inside them is `Unterminated string` — backticks do both |
| `"""multi-line"""` | `` `multi-line` `` | backticks allow real newlines + `{expr}` |
| `[f(x) for x in xs if p(x)]` | `apply(f, where(xs, p))` | comprehension syntax → parse error with the hint `no comprehensions: apply(xs, (v) => …) to transform, where(xs, (v) => …) to filter` |
...
| `xs[1:3]`, `xs[-2:]` | `slice(xs, 1, 3)`, `slice(xs, -2)` | `[1:3]` → parse error with the hint `slice(xs, start, end?)`; `slice` takes Python-style negatives, works on lists/text/bytes |
...
| `try/except E as e:` | `try` … `recover err` | `except`/`catch` after a `try` block → parse error ``except` is not Synsema: after the try block write `recover err``; there is no `finally` (its hint: put the cleanup after the try/recover block). `err` is the message TEXT (no exception types/hierarchy). **`recover` SWALLOWS by default** — re-propagate with `raise(err)` |
...
| `type(x)` | `type_of(x)` → `"number"`, `"text"`, … (`type(x)` → parse error naming `type_of`) |
| `str(x)` / `int(s)` / `float(s)` | `text(x)` / `int(s)` (exact integer, v0.6.29+: `"-42"`, `"1_000"`, `"0x1f"`, a whole float) / `number(s)` (float). Base and fallback by name: `int("ff", base = 16)` → `255` (2–36), `int("x", fallback = 0)` → `0`; `int("ff", 16)` → error naming both forms (an integer 2..36 as the 2nd positional is ambiguous, whatever the first argument). `int("0o17")` → `15`, `int("-0x10")` → `-16` like Python. `int(1.5)` errors — `floor`/`round`/`trunc` on purpose |
| `round(x, 2)` | `round_to(x, 2)` | `round` takes one argument; `round(x, 2)` → ``round() takes at most 1 argument, got 2 — to round to n decimals: round_to(x, n)`` |
...
| `xs.insert(i, x)` | `set xs to insert(xs, i, x)` (v0.6.29+) — negatives like Python, but an out-of-range `i` is an **error** (Python clamps) |
...
| `a // b` / `a % b` | `a // b` / `a % b` (v0.6.29+; exact for big ints, floors like Python; on floats exactly CPython's answer: `7 // 0.1` → `69.0`, `7 % 0.1` → `0.09999999999999962`) |
...
| `d.get(k, default)` / `d.pop(k)` / `{**a, **b}` | `get(m, k, default)` / `remove(m, k)` (new map) / `merge(a, b)` (new map, right wins) (v0.6.29+). `d.get("a")` → `Map has no key 'get' — … Synsema has no methods: get(m, key, default)` |
| `d.keys()` / `d.values()` / `d.items()` | `keys(m)` / `values(m)` / `items(m)` → `[{key, value}, …]` (v0.6.29+) |
| `json.dumps(x)` / `json.loads(s)` | `json_encode(x)` / `json_decode(s)` (pure, no import; big integers exact up to 4300 digits like Python's `int`, `1e400` → error instead of `inf`; a leading UTF-8 BOM is skipped; `NaN`/`Infinity` are an error unless `json_decode(s, allow_nan = true)` — Python's `json.loads` accepts them silently) |
| `"%d" % x` / `"{}".format(x)` | `` `{x}` `` / `fmt("{x}", {"x": x})` | `%`-formatting → error `there is no %-formatting: use a template` |
...
| `for k, g in df.groupby("r"):` | `each g in group_by(rows, "r")` … `g.key` / `g.items` | `group_by` returns `[{key, items}]`, NOT a dict (`keys(group_by(…))` → an error that explains the shape); keys group by `==` (`{"a": 1, "b": 2}` = `{"b": 2, "a": 1}`; `1` = `1.0`, but ≠ `"1"`); a column no row has → error |
| `df.merge(o, on="id", how="left")` / polars `join(how="semi")` | `join(rows, o, "id", "left")` / `join(rows, o, "id", "semi")` | `how` = `"inner"`/`"left"`/`"right"`/`"outer"`/`"semi"`/`"anti"`; a `nothing` key never matches (SQL); clashing columns get `_right` (pandas: `_x`/`_y`); `join(xs, sep)` with 2 args is still the text join |
...
| `s.count()` / `s.isna().sum()` | `count(xs)` / `count_missing(xs)` | `count` counts the **present** values (not `nothing`), like pandas/SQL |
| `df.dropna()` / `df.dropna(subset=["m"])` / `df.fillna(0)` / `df.fillna({"m": 0})` | `drop_missing(rows)` / `drop_missing(rows, "m")` / `fill_missing(rows, 0)` / `fill_missing(rows, {"m": 0})` | missing = `nothing`; NaN is separate: `fill_nan(xs, 0)`. The pandas names used as builtins (`dropna`, `fillna`, `isna`, `isnull`, `groupby`, `value_counts`, `read_csv`, `to_csv`) fail naming the Synsema one (`Undefined variable: 'dropna' — in Synsema: drop_missing(rows)`) |
...
| `np.corrcoef(x, y)[0, 1]` / `np.cov(x, y)[0, 1]` / `np.linalg.lstsq(A, b)` / `np.polyfit(x, y, 1)` | `corr(x, y)` / `cov(x, y)` / `lstsq(A, b)` / `polyfit(x, y, 1)` + `polyval(c, x)` | `cov` is sample by default, like numpy's `cov`; `lstsq` is numpy's (SVD, minimum-norm on rank-deficient data); an undetermined `polyfit` is an error, not a `RankWarning` |
| `len(arr)` / `arr.size` / `arr[-1]` / `arr[1:3]` | `length(a)` / `size(a)` / `a[-1]` / `slice(a, 1, 3)` | — |
| `rng = np.random.default_rng(42)`; `rng.random()`, `rng.integers(1, 7)`, `rng.normal(0, 1)`, `rng.permutation(xs)`, `rng.choice(xs, 5, replace=False)`, `rng.choice(xs)`, `rng.spawn(4)` | `let g be rng(42)`; `g()`, `random_int(g, 1, 6)`, `random_normal(g, mean = 0, std = 1)`, `shuffle(g, xs)`, `sample(g, xs, 5)`, `choice(g, xs)`, `rng_spawn(g, 4)` | **the same numbers as numpy, bit for bit** (v0.6.29+); `random_int` is **inclusive** on both ends (`hi + 1` in numpy); pure, no capability. `parallel_map(f, rng_spawn(g, n))` = numpy's children in worker processes; the same generator in two items is an **error** (numpy copies it silently and every worker draws the same numbers) |
| `random.random()` | `random()` + `require random` | the unseeded form reads OS entropy |
| `pd.to_datetime("2026-01-03")` / `datetime.date(2026, 1, 3)` | `datetime("2026-01-03")` / `date(2026, 1, 3)` | three types: `date`, `datetime` (instant + IANA zone or fixed offset; `datetime("…+05:30")` keeps the offset, like `fromisoformat`), `duration` |
...
| `dt.astimezone(ZoneInfo("UTC"))` / `ZoneInfo("Europe/Madrid")` | `to_timezone(dt, "UTC")` / the zone name as text | building a nonexistent local time (DST gap) is an error, not shifted; `add_days`/`add_months` landing in a gap move forward (Temporal "compatible") |
...
| `for r in rows: r["x"] = 1` updates the rows | `each r in rows` + `set r["x"] to 1` changes the loop's **copy**, not the row (`synsema check` warns). Write `set rows to apply((r) => merge(r, {"x": 1}), rows)` or by index: `set rows[i]["x"] to 1` |
...
- Int/float comparison is exact (`2**53 + 1 == 9007199254740992.0` → false), `json_decode` keeps big integers exact, `0x…`/`0o…`/`0b…` literals exist, `int()` exists, float `//` and `%` give CPython's exact results (v0.6.29+).
