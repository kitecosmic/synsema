...
store with the life of the server). The same goes for a seeded generator: one built at the top
level would restart the same sequence in every request, so using it in a handler is an error
...
any client can inject it and a generic proxy passes it through (host header injection →
