...
| `desk.exe --engine version` captured from PowerShell dies with `failed printing to stdout … (os error 232)` | PowerShell does not wait for GUI-subsystem programs and closes the pipe under them (`$v = & .\desk.exe …`) | Ask the console build, or run it from `cmd`; nothing to fix in the app |
