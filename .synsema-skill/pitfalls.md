...
| A lexer error about `--` on `5--1`, or on `print(x --1)`: ``… `--` starts a comment (the rest of the line, including a closing `)`, is ignored) — to subtract a negative write `x - -1` `` | Glued to a value, `--` followed by a number or `(` (`5--1`, `x--(y)`) is an error — anything else after it (`print(1)--note`, `x--note`) is a comment; inside parentheses, a value before, a digit or `(` right after and the closing `)` later on the same line (`print(x --1)`) is flagged because the comment would swallow the `)` — a note after a comma in a multiline map or list (`"a": 1,  --TODO`) is a plain comment | `5 - -1` / `x - -1` for arithmetic, `5 -- note` for a comment |
...
| A loop that used to stop at `Loop exceeded maximum iterations` now runs forever | `while` has no iteration cap (it was 1,000,000; the wasm build keeps it, since its host cannot interrupt a loop) | Make the condition change; bound it yourself if you relied on the cap |
| Output of a long `synsema run` appears as it happens | `print`, `log` and `show` under `run` are written line by line, in program order (`print` was held until the end) | Nothing — `flush()` is no longer needed there |
...
| A CSV field that was `""` is now `nothing` (and `text + nothing` errors, `== ""` is false) | `csv_parse` reads an **empty field as `nothing`** — missing data; a quoted `""` is still the empty text. `synsema check` warns on `== ""`/`!= ""` in a program that reads CSV | `is_missing(x)` / `fill_missing(rows, "")` if you want the old empty text; `drop_missing(rows, "col")` to drop them |
...
| `Invalid set target: it must start from a variable` | `set get(m, "a")["b"] to 1` wrote into a temporary | Write the path from the variable: `set m["a"]["b"] to 1` |
| `synsema check`: `` `set r[…]` changes the loop's copy, not the item in the list `` | `each r in rows` gives a **copy** of each row (value semantics) — `set r["x"] to 1` never reaches `rows` | `set rows to apply((r) => merge(r, {"x": 1}), rows)`, or by index: `set rows[i]["x"] to 1` |
| `no row has a column "vv" (columns: "g", "v")` from `group_by`/`summarize`/`sum_of`/`join`/`pivot` | A column name that no row has was a silent all-`nothing` column | Fix the name. A column present in only some rows is still `nothing` where absent |
| Groups merged or split differently (`count_by`, `unique`, `mode`, `group_by`) | Group keys use `==`: `{"a": 1, "b": 2}` and `{"b": 2, "a": 1}` are one group, `1`/`1.0`/`true` one, `"1"` another, all NaN one | Convert the key on purpose (`text(x)`) if you want texts and numbers apart/together |
| `join` returns rows with columns you did not have / `how = "right"`/`"semi"`/`"anti"` | Every output row carries every column (`nothing` where unmatched); `nothing` keys never match; `right`/`semi`/`anti` are new | Drop columns after the join if you need fewer |
| `summarize: the aggregate "g" has the name of a key column` / `pivot: the values 1 and "1" … would both become the column "1"` | Both used to overwrite a column silently | Rename the aggregate; convert the pivot column so the values differ |
| `lstsq` answers where it used to error; `polyfit` errors where numpy only warns | By design: `lstsq` gives numpy's answer (SVD, the **minimum-norm** solution on rank-deficient data — the least-squares problem has one, so there is nothing to refuse); `polyfit` refuses a fit the data do not determine, because a curve's coefficients then mean nothing | Check the rank yourself if a unique answer matters; lower the `polyfit` degree |
| `argmin`/`argmax` on a list with `nothing` stopped erroring | `nothing` is skipped (the index is still the position in the original list); a NaN wins; all `nothing` → error | — |
| `json_decode("1e400")` errors (it gave `inf`); an integer of 4301+ digits errors | JSON has no infinity; the digit cap is Python's (conversion is quadratic) | Send such values as text; `json_decode(x, nothing)` to validate untrusted input |
| `evm_tx_raw: "value" does not match "fields" — the map was modified after evm_tx/evm_tx_create` | `evm_tx_raw` recomputes the digest and checks every echoed number; a creation must carry `from` | Build the map again with `evm_tx`/`evm_tx_create` instead of editing it |
| A URL with `user:pass@` now authenticates | `http_*` sends `Authorization: Basic …` from the URL's userinfo (unless you pass `Authorization`) | Remove the userinfo if the server must not get it |
...
| `int(x, 10): the second argument is the fallback value, not the base — for base 10 write int(x, base = 10); for a fallback of 10, name it: int(x, fallback = 10)` | Any integer 2..36 as the 2nd positional argument is ambiguous (base for a Python reader, fallback for the engine) — whatever `x` is, so the result never depends on the data (`int(nothing, 10)` errors too) | Name it: `int(s, base = 10)` or `int(s, fallback = 10)`. Outside 2..36 (`int(s, 100)`, `int(s, nothing)`) it is still a fallback |
| ``Invalid number literal: 1__0 — `_` goes only between two digits`` (also `1_`, `0x1_`, `1_.5`, `1e3_`) | `_` in a number literal goes only between digits (or right after `0x`/`0o`/`0b`) | `1_000`, `1_000.5`, `0x_ff` |
| `"\u00e9"` prints `é` instead of the backslash text (`synsema check`: `changed in v0.6.29: \uXXXX in a string is now an escape`) | `\uXXXX` (and `\u{…}` outside backticks) are escapes now | For a literal backslash write `\\u`. An incomplete `\u` stays literal (`"C:\users"`); `\x` is never an escape (`"C:\build\x64"`, regex `"a\x2eb"`) |
| `7 // 0.1` gives `69.0` (was `70.0`), `7 % 0.1` gives `0.09999999999999962` (was `0.0`) | Float `//` and `%` are exactly CPython's/numpy's (computed on the real binary values; `0.1` is a bit more than 0.1) — on arrays too | Nothing, unless a test pinned the old answers. For exact decimal arithmetic use decimals (`7d // 0.1d`) |
| `median`/`var`/`std`/`quantile`/`percentile` of decimals return a **decimal** (`var([1000000000000000000001d, 1000000000000000000003d])` → `2`; a float computation gives `0.0`), even when tiny (`std` of decimals that differ by 1e-15 → `0.000000000000001527525231651946668862682398`) | Computed exactly, like Python's `statistics` (integers of any size included: `median([1d, 2d, 10**30])` → `2`); always a decimal — exact, or 28 significant digits | `float(x)` if a float is wanted |
| `1d + 10**30` is a decimal with 31 digits; `1.1d ** 30` prints 30 decimals; `floor(2.5d)` is `2` | Decimals have arbitrary precision (v0.6.29+): `+ - * // % **` are exact at any size and a decimal op always gives a decimal (it used to fall back to float past 28 digits); `/` rounds to 28 significant digits; `floor`/`ceil`/`round`/`trunc` give the exact integer (they returned the decimal unchanged) | `round_to(x, n)` to fix the decimals you show |
| `join: item 2 is nothing — convert it on purpose (text(x)), or drop the missing ones first` (also a list/map/bytes/task item) | `join(xs, sep)` refuses what `+` with text refuses; numbers/bools are still written | `join(where(xs, (x) => x != nothing), sep)`, or `text(x)` on purpose |
| `chunk: size must be an integer, got 1.5` (also `2.0`) | The chunk size must be an integer value | `chunk(xs, 2)` — `int(x)` / `floor(x)` on purpose |
| `join: the right column "b" would be named "b_right", which the left side already has` | A clashing suffix used to overwrite a column silently | Rename a column before the join |
| `sql_exec: this statement returns rows (RETURNING) — run it with sql(statement, params) to get them (nothing was executed)` | On SQLite, `sql_exec` would write the row and then fail (Postgres/MySQL run it and drop the rows) | `sql("INSERT … RETURNING id", params)` — nothing was written, it is safe to rerun it with `sql` |
| `parquet_read: the file has N cells (rows × columns); the limit is 50000000` / `… refusing to allocate it` | A small file can no longer make the engine allocate gigabytes: 50 M cells, 256 MiB per page, max(1 GiB, 64 × file size) — checked before allocating | For a big file you trust: `parquet_read(b, {"max_cells": n, "max_bytes": n})` |
| `parallel_map: the same generator reaches more than one item — each worker would draw the same numbers` | A generator crosses to a worker as a copy of its state; one generator in two items would repeat its numbers | `parallel_map(f, rng_spawn(g, length(items)))` — one child per item |
| `http_get(): the URL has no host (expected scheme://host/…)` (`http://alice:pw@/rpc`, `http://h:1:2/`) | Such URLs used to reach the capability check and echo the credential into the error and the audit | Fix the URL; the error never repeats it |
| `json_decode: invalid JSON: NaN/Infinity is not JSON (a NaN passes every x <= 0 check)` | `NaN`/`Infinity` are not JSON; a NaN amount would pass `amount <= 0` | If the data is yours (`json_encode` writes them): `json_decode(text, allow_nan = true)` (also `jsonl_decode`). A leading BOM is skipped; `1e400` is an error |
| `summarize` over rows where some rows lack the aggregated column no longer errors | The column is checked against the whole table, not each group (a group with no values: `sum_of` → `0`, others → `nothing`) | Only a column that no row has is an error |
...
| `` `else` is not a Synsema statement: the fallback branch is `otherwise` `` — the same for `elif`, `pass`, `break`, `continue`, `except` (v0.6.29+; it used to be `Unexpected token: INDENT`) | A Python keyword alone on its line | `otherwise` / `otherwise when <cond>` / leave the branch out / `stop` / wrap the rest of the loop body in `when` (there is **no `continue`**) / `try` … `recover err` |
| ``Unexpected character: '?' — no `c ? a : b`: the inline form is `when c then a otherwise b` `` | A C-style ternary | `when c then a otherwise b` |
| ``Unexpected character: '&' — the logical operators are `and` / `or` `` (also `\|`); ``Unexpected character: ';' — one statement per line`` | `&&`/`\|\|`/`;` reflexes | `and` / `or`; one statement per line |
| `Expected RBRACKET, got IDENTIFIER — no comprehensions: apply(xs, (v) => …) to transform, where(xs, (v) => …) to filter` | `[v * 2 for v in xs]` | `apply(xs, (v) => v * 2)`; filter with `where` |
| `round() takes at most 1 argument, got 2 — to round to n decimals: round_to(x, n)` | Python's `round(x, 2)` | `round_to(x, 2)` |
| `Undefined variable: 'dropna' — in Synsema: drop_missing(rows)` (also `fillna`, `isna`, `isnull`, `groupby`, `value_counts`, `read_csv`, `to_csv`) | A pandas name | The builtin the error names — table: [python-diff.md](python-diff.md) § Data analysis |
| `internal error in the engine (a bug, not your program — please report it): …` | The engine itself failed (a panic) — before v0.6.29 this read as a probable native stack overflow | Report it with the program that triggers it; nothing in your code to change |
...
- **Waiting for a human with the default timeout**: `term_recv(h)` / `select([term])` time out
  after 30 s and return `nothing` — pass a timeout and loop again on `nothing` (Synsema has no
  `continue`: put the handling under `when ev != nothing`).
...
| `set xs to append(xs, x)` in a loop is quadratic | O(1) amortized since v0.6.29 when nobody else holds the list (100k appends: minutes → under a second) — also through a path (`set s.items to append(s.items, x)`, `set xs[0] to append(xs[0], x)`), and `set m[k] to v` in a loop is linear | Keep writing it; don't keep an extra alias to the list inside the loop (a shared list is copied on write) |
...
| `int("ff", 16)` parses hex | Error (v0.6.29+): `the second argument is the fallback value, not the base — for base 16 write int(x, base = 16); for a fallback of 16, name it: int(x, fallback = 16)` (any integer 2..36; outside that range a positional is still a fallback) | `int("ff", base = 16)` → `255` (or `int("0xff")`); `int(1.5)` errors on purpose (`floor`/`round`/`trunc`) |
...
| An empty CSV cell is `""` | It is **`nothing`** (missing) since v0.6.29, typed or not; only a quoted `""` is the empty text (and `csv_encode` writes `""` for it, so both round-trip) | `drop_missing(rows, "col")` / `fill_missing(rows, {"col": 0})` on purpose |
...
| A repeated column after `join` overwrote mine | The right side's non-key column gets the suffix **`_right`** (`a`, `a_right`); if `a_right` is already taken (on the left, or by another right column) it is an **error**, never an overwrite | Rename or `collect` the one you want after the join; rename before it when the suffixed name clashes |
...
| `add_days(datetime("2026-03-28T12:00:00+01:00"), 1)` follows Madrid's DST change | A fixed offset (`+01:00`) is kept, but it has **no daylight saving**: the next day is still `+01:00`, one hour off Madrid's clock | For a city, use its IANA zone: `datetime("2026-03-28T12:00:00", "Europe/Madrid")`, or `to_timezone(dt, "Europe/Madrid")` |
| `date("31/01/2026")` | Error — `date(text)` is ISO only (`YYYY-MM-DD`) | `parse_date("31/01/2026", "%d/%m/%Y")` |
| `let h be g` gives an independent copy of the generator `g` | A generator is a process, not a value (as in numpy): `g` and `h` are the same generator and take turns on one sequence | Independent streams: `rng_spawn(g, n)` (numpy's `g.spawn(n)`) or separate seeds |
| A generator drawn inside `parallel_map` workers / `serve` requests advances like in a loop | A generator crosses into a worker as a **copy** of its state (the caller's does not advance); the same generator in two items is an error; a top-level generator used in a handler or a worker (by name or inside a global map) is an error (`the generator rng(1) was created at the top level; here it would restart the same sequence in every request/worker …`) instead of silently repeating the same numbers | Workers: `parallel_map(f, rng_spawn(g, length(items)))`. Requests: build it inside the handler from something request-specific (`rng(int(params.seed))`), or `random()` + `require random` when it need not be reproducible |
