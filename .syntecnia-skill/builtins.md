...
- `render(template_path, data?)` → `text/html` from a template file. A hole `{ x }` is a **data field** (a single name — even a reserved word like `type`) or an **expression** (`{ format_time(created) }`). Values are auto-escaped (XSS-safe); `{ raw expr }` opts out; `{ each x in xs }…{ end }` and `{ when c }…{ otherwise }…{ end }` reuse Syntecnia flow. cwd-relative + traversal-blocked; `render("literal")` templates are validated at startup; errors carry `file:line`. See serve.md.
